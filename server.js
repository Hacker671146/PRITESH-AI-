...
let mockPeriod = 100000;
let mockNumbers = [5, 3, 7, 8, 2, 6, 9, 4, 1, 0];

function generateMockData() {
    // Generate 3 random numbers for testing
    const numbers = [];
    for (let i = 0; i < 3; i++) {
        numbers.push(Math.floor(Math.random() * 10));
    }
    return numbers.map(n => ({
        period: String(mockPeriod + Math.floor(Math.random() * 100)),
        number: n
    }));
}
...
async function fetchLiveData() {
    try {
        const ts = Date.now();
        const res = await axios.get(API_URL, { params: { ts }, headers: HEADERS, timeout: 5000 });
        const data = res.data;
        ...
        if (currentPeriod !== lastPeriod) {
            ...
            if (activeResult && predictedPeriod) {
                ...
                history.push({
                    ...
                    predicted: { type: predictedType, digit: predictedDigit, confidence: activeResult.confidence },
                    actual: { number: currentNumber, type: actualType },
                    win: win,
                ...
                console.log(`[${new Date().toISOString()}] Period ${predictedPeriod} → ${predictedType}(${predictedDigit}) | Actual ${actualType}(${currentNumber}) → ${win ? 'WIN ✅' : 'LOSS ❌'}`);
                console.log(`📊 Total=${stats.totalPredictions}, Wins=${stats.totalWins}, Losses=${stats.totalLosses}, Acc=${stats.accuracy.toFixed(1)}%`);
            }

            const n1 = data[0].number, n2 = data[1].number, n3 = data[2].number;
            const result = predictor.predict(n1, n2, n3);
            const nextPeriod = String(parseInt(currentPeriod, 10) + 1);
            predictedPeriod = nextPeriod;
            activeResult = { period: nextPeriod, prediction: result.prediction, digit: result.digit, confidence: result.confidence };
...
app.get('/', (req, res) => {
    try {
        res.json({
            ...
            status: activeResult ? 'active' : 'initializing'
        });
    } catch (err) {
...
app.get('/history', (req, res) => {
    try { res.json(history); } catch (err) { res.status(500).json({ error: 'Error' }); }
});
...
app.get('/ping', (req, res) => {
    res.json({ status: 'ok', timestamp: new Date().toISOString(), hasPrediction: !!activeResult });
});
...
app.listen(PORT, () => {
    ...
    console.log(`👉 GET /ping to check health`);
    console.log(`👉 GET / for full data (JSON)`);
    console.log(`⏳ First prediction will appear in 2-3 seconds...`);
});
