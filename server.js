...
processLoop();
setInterval(processLoop, 3000); // Check every 3 seconds
...
app.get('/', (req, res) => {
    try {
        const response = {
            status: isLiveDataAvailable ? 'live' : 'waiting_for_data',
...
app.listen(PORT, () => {
    ...
    console.log(`📡 Status: ${isLiveDataAvailable ? 'LIVE' : 'WAITING FOR DATA'}`);
    ...
    console.log(`\n⏳ Waiting for live data... First prediction in ~3 seconds\n`);
});
